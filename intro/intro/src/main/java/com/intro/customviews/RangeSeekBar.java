 ...
 *            <p/>
 *            <p/>
 * @modified by Ankit Kumar & look for // change by Ankit
 */
public class RangeSeekBar<T extends Number> extends ImageView {
	...
	private final Bitmap thumbImage = BitmapFactory.decodeResource(
			getResources(), R.drawable.slider);// thumb
	private final Bitmap thumbPressedImage = BitmapFactory.decodeResource(
			getResources(), R.drawable.slider);// thumb_hit
	 ...
	 */
	public static final int DEFAULT_COLOR = Color.parseColor("#00b4d9");// Color.argb(0xFF,
																		// 0x33,
																		// 0xB5,  
																		// 0xE5);//changed by ankit
	/**
	...
	private static final int DEFAULT_TEXT_SIZE_IN_DP = 22;
	...
	protected synchronized void onDraw(Canvas canvas) {
		...
		paint.setColor(Color.parseColor("#D21768"));
		...
		int colorToUseForButtonsAndHighlightedLine = selectedValuesAreDefault ? Color
				.parseColor("#00b4d9") : // default values
				Color.parseColor("#50B5C7"); // non default, filter is active
		...
		// if (!selectedValuesAreDefault) { // change by Ankit
		paint.setTextSize(mTextSize);
		...
		if (Integer.valueOf(maxText) >= 55) {  /// change done by Ankit to display String value

			maxText = "55+";
		}
		if (Integer.valueOf(minText) >= 55) {
		...
		if (!mSingleThumb) {
			canvas.drawText(minText, normalizedToScreen(normalizedMinValue)
					- minTextWidth * 0.5f, mTextSize, paint);// mDistanceToTop +
		}
